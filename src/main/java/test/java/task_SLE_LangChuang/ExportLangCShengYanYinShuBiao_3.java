 ...
 * Time:下午4:49
 */
public class ExportLangCShengYanYinShuBiao_3 {
    static int k= 2;
    static int m=14;
    ...
    public static void mainFunction(int kValue,int mValue,LangCShengYanYinShuPublicInfo obj) throws Exception
    {
        ...
        langCShengYanYinShuPublicInfo.fillExcelTitle(sheet,BaseInfo_Title_ListValue_DBCondition.titleLCShengYanYinShuB3,false);
        writeToExcel(sheet);
        saveExcelTool.saveExcel("交付/狼疮肾炎因素表-分析3用-"+k+"-"+m+".xlsx");
        saveNotInYinSuBiao(k,m);  //获取分析2表未在2表但在pid验证的pid
        langCShengYanYinShuPublicInfo.getCacliteExcel("狼疮肾炎因素分析计算删除列.xlsx","交付/狼疮肾炎因素表-分析3用-"+k+"-"+m+".xlsx",BaseInfo_Title_ListValue_DBCondition.titleLCShengYanYinShuB3,
                "交付/狼疮肾炎因素表-分析3计算用-"+k+"-"+m+".xlsx","");
    }
    ...
    private static void saveNotInYinSuBiao(int k,int m) throws Exception
    {
        ...
        String fileName= LocalHostInfo.getPath()+"交付/狼疮肾炎因素表-分析3用-"+k+"-"+m+".xlsx";
        ...
        for(Map.Entry<String,String> map:mapPID.entrySet())
        {
            if(!mapYinSuTable.containsKey(map.getKey()))
            {
                ...
                row.createCell(0).setCellValue("狼疮肾炎因素分析3");
                row.createCell(1).setCellValue(map.getKey());
            }
        }
        saveExcelTool.saveExcel("交付/移除组PID列表-分析3-"+k+"-"+m+".xlsx");
    ...
    private static void fillLeftColumForExcel(Row row,int columNum,String strPid,String strLCShengYanTime)
    {
        for (Map.Entry<String,ArrayList> map:langCShengYanYinShuPublicInfo.mapLeiJiFenZu.entrySet())
        {
            ...
            else
            {
                ...
                row.createCell(columNum++).setCellValue(dd.getString("时间天"));
            }
        }
