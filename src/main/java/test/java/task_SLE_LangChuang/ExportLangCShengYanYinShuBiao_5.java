 ...
 * Time:下午3:09
 */
public class ExportLangCShengYanYinShuBiao_5 {
    static int k= 2;
    static int m=14;
    static int p=-180;
    static JSONObject document=null;
    static LangCShengYanYinShuPublicInfo langCShengYanYinShuPublicInfo;
    public static void mainFunction(int kValue,int mValue,int pValue,LangCShengYanYinShuPublicInfo obj) throws Exception
    {
        k=kValue;
        p=pValue;
        m=mValue;
        ...
        langCShengYanYinShuPublicInfo.fillExcelTitle(sheet,BaseInfo_Title_ListValue_DBCondition.titleLCShengYanYinShuB5,false);
        writeToExcel(sheet);
        saveExcelTool.saveExcel("交付/狼疮肾炎因素表-分析5用-"+k+"-"+m+p+".xlsx");
        saveNotInYinSuBiao(k,m);  //获取分析2表未在2表但在pid验证的pid
        langCShengYanYinShuPublicInfo.getCacliteExcel("狼疮肾炎因素分析计算删除列.xlsx","交付/狼疮肾炎因素表-分析5用-"+k+"-"+m+p+".xlsx",BaseInfo_Title_ListValue_DBCondition.titleLCShengYanYinShuB5,
                "交付/狼疮肾炎因素表-分析5计算用-"+k+"-"+m+p+".xlsx","");
    }
    ...
    private static void saveNotInYinSuBiao(int k,int m) throws Exception
    {
        ...
        String fileName= LocalHostInfo.getPath()+"交付/狼疮肾炎因素表-分析5用-"+k+"-"+m+p+".xlsx";
        ...
        for(Map.Entry<String,String> map:mapPID.entrySet())
        {
            if(!mapYinSuTable.containsKey(map.getKey()))
            {
                ...
                row.createCell(0).setCellValue("狼疮肾炎因素分析5");
                row.createCell(1).setCellValue(map.getKey());
            }
        }
        saveExcelTool.saveExcel("交付/移除组PID列表-分析5-"+k+"-"+m+p+".xlsx");
    ...
    private static void writeToExcel(SXSSFSheet sheet)
   {
       ...
       for(Map.Entry<String,JSONObject> map :langCShengYanYinShuPublicInfo.mapLCShengYanPID.entrySet())
       {
          ...
          // row.createCell(5).setCellValue(strGuanCQZhongDian);
           ...
           fillLeftColumForExcel(row,13,strPID, DateFormat.getNextDay(strGuanCQZhongDian,p));
       }
